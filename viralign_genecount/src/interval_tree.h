...
// interval tree adapted from https://github.com/ekg/intervaltree
#pragma once 
...
class IntervalTree {
 public:
  ...
  IntervalTree(interval_vector&& ivals, std::size_t depth = 16,
               std::size_t minbucket = 64, std::size_t maxbucket = 512,
               Scalar leftextent = 0, Scalar rightextent = 0)
      : left(nullptr), right(nullptr) {
    --depth;
    const auto minmaxStop =
        std::minmax_element(ivals.begin(), ivals.end(), IntervalStopCmp());
    const auto minmaxStart =
        std::minmax_element(ivals.begin(), ivals.end(), IntervalStartCmp());
    if (!ivals.empty()) {
    ...
    if (depth == 0 || (ivals.size() < minbucket && ivals.size() < maxbucket)) {
      ...
      if (leftextent || rightextent) {
        ...
        rightp = std::max_element(ivals.begin(), ivals.end(), IntervalStopCmp())
                     ->stop;
      }
      ...
      if (!lefts.empty()) {
        left.reset(new IntervalTree(std::move(lefts), depth, minbucket,
                                    maxbucket, leftp, center));
      }
      if (!rights.empty()) {
        right.reset(new IntervalTree(std::move(rights), depth, minbucket,
                                     maxbucket, center, rightp));
      }
    }
    assert(is_valid().first);
  }
  ...
  void visit_near(const Scalar& start, const Scalar& stop,
  ...
  template <class UnaryFunction>
  void visit_overlapping(const Scalar& start, const Scalar& stop,
                         UnaryFunction f) const {
  ...
  template <class UnaryFunction>
  void visit_contained(const Scalar& start, const Scalar& stop,
                       UnaryFunction f) const {
    auto filterF = [&](const interval& interval) {
  ...
  void findOverlappingValues(const Scalar& start,
                                  const Scalar& stop, std::vector<const Value*>& results) const {

    visit_overlapping(start, stop, [&](const interval& interval) {
      results.emplace_back(&interval.value);
    });
  }
  ...
  std::pair<bool, std::pair<Scalar, Scalar>> is_valid() const {
    const auto minmaxStop = std::minmax_element(
        intervals.begin(), intervals.end(), IntervalStopCmp());
    const auto minmaxStart = std::minmax_element(
        intervals.begin(), intervals.end(), IntervalStartCmp());
    ...
    if (!intervals.empty()) {
      result.second.first =
          std::min(result.second.first, minmaxStart.first->start);
      result.second.second =
          std::min(result.second.second, minmaxStop.second->stop);
    }
    if (left) {
      ...
      result.second.second =
          std::min(result.second.second, valid.second.second);
      if (!result.first) {
        return result;
      }
